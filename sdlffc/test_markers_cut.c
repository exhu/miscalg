...
int main(int argc, char **argv) {
  ...
  assert(context->cut.in_point >= 0.0 && context->cut.in_point < 0.001);
  assert(context->cut.out_point > 0.0);
  assert(!context->cut.modified);
  assert(!context->looping);
  assert(context->ui.overlay_mode == OVERLAY_TOP_LEFT);

  /* Simulate setting IN marker at t=4.5s and OUT marker at t=1.5s (IN > OUT) */
  context->cut.in_point = 4.5;
  context->cut.out_point = 1.5;
  ...
  assert(context->cut.in_point == 1.5);
  assert(context->cut.out_point == 4.5);

  /* Test key V cycling */
  context->ui.overlay_mode = (OverlayMode)((context->ui.overlay_mode + 1) % 3);
  assert(context->ui.overlay_mode == OVERLAY_BOTTOM_RIGHT);
  context->ui.overlay_mode = (OverlayMode)((context->ui.overlay_mode + 1) % 3);
  assert(context->ui.overlay_mode == OVERLAY_HIDDEN);
  context->ui.overlay_mode = (OverlayMode)((context->ui.overlay_mode + 1) % 3);
  assert(context->ui.overlay_mode == OVERLAY_TOP_LEFT);
