...
`sdlffcd` is a standalone D application (video player with video editing/trimming features) that uses the custom `sdlffcd_clib` C library to bridge calls to third-party C dependencies (SDL3, SDL3_ttf, FFmpeg).
...
- **C Dependencies**: All third-party C library functionality (SDL3, FFmpeg, SDL3_ttf) must be accessed via `sdlffcd_clib`. See @sdlffcd_clib/README.md.
- **D/C Header Synchronization**: `source/sdlffcd_clib.d` **must** contain D declarations that match `sdlffcd_clib/sdlffcd_clib.h` 1-to-1.
