...
/** Update letterbox/pillarbox destination rectangle only when renderer output size
 *  or video source dimensions change, caching the result in vctx. */
static void sdlffcd_update_letterbox_rect(SDL_Renderer* renderer, sdlffcd_VideoContext* vctx, int src_w, int src_h) {
    int out_w = 0, out_h = 0;
    SDL_GetCurrentRenderOutputSize(renderer, &out_w, &out_h);

    if (out_w == vctx->last_render_w && out_h == vctx->last_render_h &&
        (vctx->dst_rect.w > 0.0f || vctx->dst_rect.h > 0.0f)) {
        return;
    }

    vctx->last_render_w = out_w;
    vctx->last_render_h = out_h;
...
bool sdlffcd_video_render_frame(sdlffcd_AppContext* app, sdlffcd_VideoContext* vctx, const sdlffcd_VideoFrame* frame) {
    ...
    SDL_RenderClear(app->renderer);
    sdlffcd_update_letterbox_rect(app->renderer, vctx, vctx->texture_width, vctx->texture_height);
    SDL_RenderTexture(app->renderer, vctx->texture, NULL, &vctx->dst_rect);
    return true;
}
