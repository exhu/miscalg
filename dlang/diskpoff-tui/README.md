...
- BUG: locking unlocked partition ends in error
...
- **Partition Management**: Supports mounting, unmounting, and locking individual partitions or all partitions on a selected device.
...
- **`M`**: Mount all partitions (except swap) on the selected device, or only the selected partition.
