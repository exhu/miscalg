 ...
 * \sa SDL_SetTLS
 */
alias SDL_TLSID = SDL_AtomicInt_;
...
{
    SDL_THREAD_UNKNOWN = 0, /**< The thread is not valid */
    SDL_THREAD_ALIVE = 1, /**< The thread is currently running */
    SDL_THREAD_DETACHED = 2, /**< The thread is detached and can't be waited on */
    SDL_THREAD_COMPLETE = 3 /**< The thread has finished and should be cleaned up with SDL_WaitThread() */
}
...
extern (D) auto SDL_CreateThread(T0, T1, T2)(auto ref T0 fn, auto ref T1 name, auto ref T2 data)
{
    return SDL_CreateThreadRuntime(fn, name, data, cast(SDL_FunctionPointer) SDL_BeginThreadFunction, cast(SDL_FunctionPointer) SDL_EndThreadFunction);
}

extern (D) auto SDL_CreateThreadWithProperties(T)(auto ref T props)
{
    return SDL_CreateThreadWithPropertiesRuntime(props, cast(SDL_FunctionPointer) SDL_BeginThreadFunction, cast(SDL_FunctionPointer) SDL_EndThreadFunction);
}

enum SDL_PROP_THREAD_CREATE_ENTRY_FUNCTION_POINTER = "SDL.thread.create.entry_function";
enum SDL_PROP_THREAD_CREATE_NAME_STRING = "SDL.thread.create.name";
enum SDL_PROP_THREAD_CREATE_USERDATA_POINTER = "SDL.thread.create.userdata";
enum SDL_PROP_THREAD_CREATE_STACKSIZE_NUMBER = "SDL.thread.create.stacksize";
 ...
 * \sa SDL_SetTLS
 */
alias SDL_TLSDestructorCallback = void function (void* value);
